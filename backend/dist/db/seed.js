...
async function main() {
  ...
  `);
    ...
    const getCity = async (name) => (await db.select().from(schema_1.cities).where((0, drizzle_orm_1.eq)(schema_1.cities.name, name)).limit(1))[0]?.id;
    const stadiumsData = [
    ...
    const insertedMenuItems = await db.insert(schema_1.menuItems).values(menuItemsData).returning();
    ...
    await db.insert(schema_1.profilePermissions).values(profilePermissionsData);
