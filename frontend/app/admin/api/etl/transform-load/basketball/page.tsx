...
export default function EtlPage() {
    ...
    // Query countries via react-query at top-level (hooks must be at component root)
    ...
    }, [API_BASE, payloadCountryForClubs]);

    // --- FETCH CLUBS (use API client) ---
    ...
    useEffect(() => {
        ...
        setClubCountrySelections((prev) => {
            ...
            clubsForReview.forEach((club: any) => {
                ...
                console.log(`Processing club for review: ${club.name} (key: ${key})`);
                if (!key) return;
                if (next[key] !== undefined) return;
                console.log(`Determining default country for club "${club.name}" with payload country "${payloadCountryForClubs?.name}" and review country "${countryForReview?.name}"`);
                let defaultCountryId;
                console.log(`Club's own country_id: ${club.country_id}, Payload country ID: ${payloadCountryForClubs?.id}, Country for review suggestions: ${countryForReview?.suggestions?.map((s: any) => `${s.name}(ID:${s.id})`).join(', ')}`);
                if (club.country_id) {
                    console.log(`Using club's own country_id: ${club.country_id}`);
                    defaultCountryId = club.country_id;
                } else if (payloadCountryForClubs?.id) {
                    console.log(`Using payload country ID: ${payloadCountryForClubs.id}`);
                    defaultCountryId = payloadCountryForClubs.id;
                } else if (countryForReview?.suggestions?.length) {
                    console.log(`Using first suggestion from review country: ${countryForReview.suggestions[0].id}`);
                    defaultCountryId = countryForReview.suggestions[0].id;
                }
                console.log(`Default country ID for club "${club.name}" set to: ${defaultCountryId}`);
    ...
    const [basketballSportId, setBasketballSportId] = useState<number | null>(null);
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const bball = normalized.find((s: any) => typeof s.name === 'string' && s.name.toLowerCase().includes('basketball'));
                if (bball) setBasketballSportId(bball.id);
            } catch (e) {
                console.error('[ETL Basketball] failed to fetch sports', e);
            }
        })();
    }, []);

    useEffect(() => {
        if (basketballSportId == null) return;
        setLoadingRows(true);
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold">ETL - Basketball</h1>
            </div>
                            ...
                            <DataTable
                                columns={[
                                    ...
                                    { header: 'League', accessor: (r: any) => r.league ?? '-' , width: '240px', sortKey: 'league', sortable: true },
                                    {
                                        ...
                                        sortable: true,
                                        accessor: (r: any) => {
                                            const isNotLoaded = !r.status || (r.fetch_status && r.fetch_status !== 'done' ? false : !r.status);
                                            if (isNotLoaded && editingSeasonRowId === r.id) {
                                                return (
                                            ...
                                            <div className="flex items-center gap-3 justify-end">
                                                ...
                                                <button
                                                    onClick={async () => {
                                                        ...
                                                        setRunningLoad(true);
                                                        try {
                                                            const rowSportId = r.sport ?? basketballSportId ?? 36;
                    ...
                    <div>
                        ...
                        {(countryForReview || leagueForReview || clubsForReview.length > 0 || stadiumsForReview.length > 0) && (
                            <div data-entity-review-section className="mb-4 rounded border border-purple-300 bg-purple-50 p-4 text-sm text-purple-950">
                                ...
                                <p className="mb-4 max-w-5xl">
                                    {countryForReview
                                        ? 'The country for this data could not be determined from the payload (e.g. NBA has no country info). Please select the country this league belongs to.'
                                        : leagueForReview 
                                                    ...
                                                    <div key={club.name} className="border border-gray-300 rounded bg-white p-3 shadow-sm min-w-0">
                                                        ...
                                                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                                                            ...
                                                            <select
                                                                ...
                                                                value={selectedCountryId !== undefined && selectedCountryId !== null ? Number(selectedCountryId) : ''}
                                                                onChange={(e) => {
                                                            ...
                                                            {/* Club selector for selected country */}
                                                            <select
                                                                className="w-full md:w-72 border rounded px-2 py-1"
                                                                value={clubMappings[club.name] ?? ''}
                                                                onChange={(e) => setClubMappings((prev) => ({ ...prev, [club.name]: e.target.value ? Number(e.target.value) : undefined }))}
