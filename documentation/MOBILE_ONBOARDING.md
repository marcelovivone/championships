...
- Types: `Sport`, `League`, `Season`, `Round`, `ClubRef`, `Standing`, `Match`, `SeasonClub`, `Group`.
- Helpers: `fetchSports`, `fetchLeagues`, `fetchSeasons`, `fetchRounds`, `fetchSeasonClubs`, `fetchSeasonMatches`, `fetchStandings`, `fetchMatchesSlice`, `fetchGroups`.
...
3. **Play-ins screen** — `PlayInsScreen.tsx` using `fetchMatchesSlice` with `seasonPhase=Play-ins`.
4. **Playoffs bracket screen** — Mirror web `PostseasonBracket.tsx`; use `/v1/matches/postseason-bracket?leagueId=...&seasonId=...`.
...
----------------------
- Phase 1 (mobile — in progress): read-only views (standings, fixtures, bracket). Avoid admin flows.
- Phase 2 (mobile): authenticated read/write, push notifications, local caching.
- Parallel work: agent development can resume once the mobile app has stable API endpoints for its screens.
